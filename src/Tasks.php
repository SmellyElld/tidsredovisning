...
function hamtaEnskildUppgift(string $id): Response {
    ...
    //Exekvera SQL
    $stmt = $db -> prepare(
        "SELECT u.id, tid, datum, beskrivning, aktivitetid, namn 
        FROM uppgifter u
        INNER JOIN aktiviteter a ON aktivitetId=a.id
        WHERE u.id = :id");
...
function sparaNyUppgift(array $postData): Response {
    //Kontrollera indata
    $felmedelande = kontrolleraIndata($postData);
    if(count($felmedelande) > 0){
        $retur = new stdClass();
        $retur -> error = $felmedelande;
        array_unshift($retur -> error, "Bad request");
        return new Response($retur, 400);
    }
    //Koppla databasen
    ...
    $stmt = $db -> prepare("INSERT INTO uppgifter (datum, tid, beskrivning, aktivitetid) VALUES (:datum, :tid, :beskrivning, :aktivitetid)");
    $stmt -> execute(["datum" => $postData["date"], "tid" => $postData["time"], "beskrivning" => trim(filter_var($postData["description"] ?? "", FILTER_SANITIZE_SPECIAL_CHARS)), "aktivitetid" => $postData["activityId"]]);
...
function uppdateraUppgift(string $id, array $postData): Response {
    ...
    $stmt = $db -> prepare(
        "UPDATE uppgifter 
        SET datum = :date, tid = :time, aktivitetid = :activityid, beskrivning = :description
        WHERE id = :id ");
    $stmt -> execute(["date" => $postData["date"], "time" => $postData["time"], "activityid" => $postData["activityId"], "description" => $postData["description"] ?? "", "id" => $kontrolleratId]);
